...
// Swap routes: servir UI en /swagger y JSON en /swagger-ui/{documentName}/swagger.json
app.UseSwagger(c =>
{
    // Ahora el JSON se servirá en /swagger-ui/{documentName}/swagger.json
    c.RouteTemplate = "swagger-ui/{documentName}/swagger.json";
});

app.UseSwaggerUI(c =>
{
    // Apunta al JSON en /swagger-ui/v1/swagger.json
    c.SwaggerEndpoint("/swagger-ui/v1/swagger.json", "Practica 3 API V1");
    // Servir la UI en /swagger
